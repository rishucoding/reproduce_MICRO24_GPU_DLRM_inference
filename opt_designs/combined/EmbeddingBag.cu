...
__global__ void reduce_L2_size(int const* d_dummy_vec, size_t size_dummy)
{
    size_t const idx{blockDim.x * blockIdx.x + threadIdx.x};
    size_t const stride{blockDim.x * gridDim.x};
    for (size_t i{idx}; i < size_dummy; i += stride)
    {
        //insert the L2 prefetch inline PTX code
        asm volatile (
            "prefetch.global.L2::evict_last [%0];" ::"l"(&d_dummy_vec[i])
        :);
    }
}

//TODO: kernel implementation is hardcoded to embedding dimension --> fix: make it parameterized (like ED={16,32,64,128,256}) 
template <typename scalar_t, typename index_t>
__global__ void prefetch_pin_L2(const scalar_t *weight, int64_t featureSize, int64_t weight_stride0, int64_t const* d_hot_idx_vec, size_t pinning_size)
{
    ...
    if(Yidx < featureSize){
        for (int i = Xidx; i < pinning_size; i += stride){
            int64_t const lv_hot_idx = d_hot_idx_vec[i];
...
__global__ void EmbeddingBag_updateOutputKernel_sum_mean(
  ...
  for (int64_t chunk = chunkOffset; chunk < numChunks; chunk += chunkStride) {
    ...
    if (featureDim < featureSize) {
      ...
      int64_t bag_size_ = 0;
      scalar_t lv0, lv1;//, lv2,lv3;//, lv4; //temp registers to store the prefetch values
      int ctr = 0;
      for (int64_t emb = begin; emb < end; emb++) {
        int ctr_mod;
...
_embedding_bag_cuda(const Tensor &weight, const Tensor &indices_,
  ...
  //Step#1: Load top indices file and to create hot indices array
  const char* fname = std::getenv("DATASET_FILE_PATH");
  if (fname == nullptr) {
    std::cout << "^^^^^ Environment variable DATASET_FILE_PATH not found." << std::endl;
  }
  std::string file_path(fname);
  std::ifstream file(file_path);
  //std::ifstream file("/scratch/rzj5233/problem2/problem2/datasets/rj_30000_high_hot_indices.txt");
  //std::ifstream file("/data/rzj5233/problem2/problem2/datasets/rj_60K_high_hot_indices.txt");
  //std::ifstream file("/scratch/rzj5233/problem2/problem2/datasets/rj_60K_medium_hot_indices.txt");
  std::vector<int64_t> hot_indices;
  int64_t value;

  if(file.is_open()) {
      while (file >> value) {
          hot_indices.push_back(value);
      }
      file.close();
  }

  int64_t* d_hot_idx_vec;
  int64_t* h_hot_idx_vec = hot_indices.data();
  int pinning_size = hot_indices.size();
  std::cout << "Pinned size: " << pinning_size << std::endl;
  cudaMalloc(&d_hot_idx_vec, pinning_size * sizeof(int64_t));
  cudaMemcpy(d_hot_idx_vec, h_hot_idx_vec, pinning_size * sizeof(int64_t),cudaMemcpyHostToDevice);
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, weight.scalar_type(), "embedding_bag_cuda", [&] {
    AT_DISPATCH_INDEX_TYPES(indices.scalar_type(), "embedding_bag_cuda", [&] () {
      if (mode == MODE_MAX) {
        ...
        prefetch_pin_L2<scalar_t, index_t><<<grid, block_2, 0, stream>>>(weight.const_data_ptr<scalar_t>(), featureSize, weight.stride(0), d_hot_idx_vec, pinning_size);
  ...
  std::cout << std::fixed << std::setprecision(5)
              << "Latency of Embedding Bag kernel " << latency
              << " ms" << std::endl;
